  ...
  return (
    <>
      <style>{`
        ...
        .nav-logo { font-size:.95rem;font-weight:800;letter-spacing:.08em;text-decoration:none;color:#0c0c0b;margin-right:2rem;white-space:nowrap; }
        ...
        @media(max-width:768px) { .nav-a:not(.on):not(.cat-btn) { display:none; } .nav-search { display:none; } .nav-in { padding:0 1.25rem; } .nav-logo { margin-right:.5rem;font-size:.85rem; } }
      `}</style>
      <nav className="nav">
        <div className="nav-in">
          <Link href={`https://dome-auctions.com/${da}/`} className="nav-logo">DOME AUCTIONS</Link>
          <Link href={`https://dome-auctions.com/${da}/auctions/`} className="nav-a">
            {t('Alle veilingen','Toutes les ventes','Alle Auktionen','All auctions')}
          </Link>

          <div ref={catRef} style={{ position: 'relative' }}>
            <button className={`nav-a cat-btn`} onClick={() => setCatOpen(!catOpen)}>
              {t('Categorieën','Catégories','Kategorien','Categories')}
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transform: catOpen ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            {catOpen && (
              <div className="dropdown" style={{ left: 0, minWidth: '200px' }}>
                {[
                  [t('Metaalbewerking','Métallurgie','Metallverarbeitung','Metalworking'), 'metalworking'],
                  [t('Houtbewerking','Travail du bois','Holzbearbeitung','Woodworking'), 'woodworking'],
                  [t('Landbouw','Agriculture','Landwirtschaft','Agricultural'), 'agricultural'],
                  [t('Bouw','Construction','Bau','Construction'), 'construction'],
                  [t('Transport','Transport','Transport','Transport'), 'transport'],
                ].map(([label, slug]) => (
                  <Link key={slug} href={`https://dome-auctions.com/${da}/categories/${slug}/`} className="drop-a" onClick={() => setCatOpen(false)}>{label}</Link>
                ))}
              </div>
            )}
          </div>

          <Link href={`/${locale}/blog`} className="nav-a on">Blog</Link>
          <div className="nav-sp" />

          <div className="nav-search" onClick={() => window.open(`https://dome-auctions.com/${da}/auctions/`, '_blank')}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            {t('Zoek kavels...','Rechercher...','Suchen...','Search lots...')}
          </div>

          <div ref={langRef} style={{ position: 'relative' }}>
            <button className="nav-lang" onClick={() => setLangOpen(!langOpen)}>
              {localeShort[locale] || 'EN'}
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transform: langOpen ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            {langOpen && (
              <div className="dropdown" style={{ right: 0, minWidth: '160px' }}>
                {Object.entries(locales).map(([code, label]) => (
                  <Link key={code} href={`/${code}/blog`} className={`drop-a${code === locale ? ' on' : ''}`} onClick={() => setLangOpen(false)}>
                    {label}
                    {code === locale && <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0c0c0b" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>}
                  </Link>
                ))}
              </div>
            )}
          ...
          <Link href={`https://dome-auctions.com/${da}/login/`} className="nav-btn">
            {t('Inloggen','Se connecter','Einloggen','Sign in')}
          </Link>
        </div>
      </nav>
    </>

          ...
          {teamMembers.map((member) => (
            <article key={member._id} className="sector-team-card">
              <div className="sector-team-photo-wrap">
                {member.foto ? (
                  <Image
                    ...
                    alt={member.foto.alt || member.naam}
                    width={480}
                    height={400}
              ...
              <div className="sector-team-body">
                <h3 className="sector-team-name">{member.naam}</h3>
                {member.functie && (
                  <p className="sector-team-role">{member.functie}</p>
                )}
                {member.beschrijving && (
                  <p className="sector-team-desc">{member.beschrijving}</p>
                )}
                <div className="sector-team-ctas">
                  {member.meetingCalendarUrl && (
                    ...
                    >
                      {member.ctaLabel || 'Plan een gesprek'}
                    </a>
                  )}
                  {member.linkedinUrl && (
                    <a
